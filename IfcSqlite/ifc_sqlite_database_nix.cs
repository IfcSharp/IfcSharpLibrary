...
namespace ifc
{
    partial class IfcSharpSqLiteDatabase : IDisposable
    {
        ...
        public void FillFromDataSet(IfcSqliteDataSet dataSet)
        {
            ...
            try
            {
                ...
                foreach (IfcSqliteDataTable table in dataSet.Tables)
                {
        ...
        internal void InsertDataSet(IfcSqliteDataSet dataSet)
        {
            OpenConnection();
            BeginTransaction();
        ...
        private string GetColumnAttributes(IfcSqliteDataTable table)
        {
            IfcSqliteDataRow dataRow = table.Rows[0] as IfcSqliteDataRow;
            List<string> AttributePair = dataRow.Fields.Select(p => string.Format("'{0}' {1} {2}", p.Parameter.ParameterName, p.Parameter.DbType, p.Parameter.IsNullable ? "NULL" : "NOT NULL")).ToList();
        ...
        private int CreateTable(IfcSqliteDataTable table)
        {
            int returnValue = 0;
        ...
        private void InsertTable(IfcSqliteDataTable table)
        {
            if (table != null)
                foreach (IfcSqliteDataRow row in table.Rows)
                    InsertRow(table.TableName, row);
        }

        private void InsertRow(string tableName, IfcSqliteDataRow row)
        {
