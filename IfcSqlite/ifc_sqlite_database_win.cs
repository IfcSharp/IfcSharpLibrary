...
namespace ifc
{
    class IfcSharpSqLiteDatabase : IDisposable
    {
        ...
        {
            List<string> AttributePair = ((IfcSqliteDataRow)table.Rows[0]).Fields.Select(p => string.Format("'{0}' {1} {2}", p.Parameter.ParameterName, p.Parameter.DbType, p.Parameter.IsNullable ? "NULL" : "NOT NULL")).ToList();
