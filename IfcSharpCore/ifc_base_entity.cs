...
public               ImageComment(string ImgFileName,int height=0,int width=0){AddNextCommentLine(); 
                                                                               ...
                                                                               if (!NetSystem.IO.File.Exists(TargetFileName)) // insert and copy only first time if file not exist
                                                                                  {if (ImgFileName.Contains("http")) try{new NetSystem.Net.WebClient().DownloadFile(ImgFileName,TargetFileName);}catch(NetSystem.Exception){Log.Add($"ImageComment:{ImgFileName} not found", Log.Level.Exception);}
